  ...
  update-manifests:
    ...
    - name: Update Kustomize image
      run: |
        ...
        kustomize edit set image ghcr.io/\${{ github.repository }}=\${{ env.REGISTRY }}/\${{ env.IMAGE_NAME }}:\${{ github.sha }}
...
# ServiceAccount dla imagePullSecrets
...
---
# Secret dla GitHub Container Registry
apiVersion: v1
kind: Secret
metadata:
  name: ghcr-pull-secret
...
data:
  .dockerconfigjson: ewogICJhdXRocyI6IHsKICAgICJnaGNyLmlvIjogewogICAgICAiYXV0aCI6ICJZV1J0YVc0Nk1ERXpORGs9IgogICAgfQogIH0KfQ==  # Base64 encoded docker config - TO BE REPLACED
EOF
...
# 8. Skrypt do tworzenia GHCR secret
echo "🔐 Tworzenie skryptu do konfiguracji GHCR secret..."
...
# 12. Dodatkowe pliki konfiguracyjne
