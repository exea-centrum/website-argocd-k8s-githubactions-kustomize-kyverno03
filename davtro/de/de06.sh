...
mkdir -p $REPO_NAME
cd $REPO_NAME
mkdir -p src templates static/css static/js manifests/base manifests/production .github/workflows argocd policies monitoring terraform
...
# 5. GitHub Actions - POPRAWIONA
print_step "Konfiguracja GitHub Actions (poprawiona wersja)..."
      ...
      - name: Ensure static directory exists
        run: |
          ...
          if [ ! -f "static/css/style.css" ]; then
            echo "body { margin: 0; }" > static/css/style.css
          fi
...
# Deployment - POPRAWIONY (bez secretKeyRef)
...
# PostgreSQL - POPRAWIONY (bez secretKeyRef)
...
# Kustomization Production - POPRAWIONY (z monitoringiem)
...
images:
  - name: ghcr.io/$REPO_OWNER/$REPO_NAME
    newName: ghcr.io/$REPO_OWNER/$REPO_NAME
    newTag: latest
EOF
...
data:
  prometheus.yml: |
    ...
    - job_name: 'website-app'
      static_configs:
      - targets: ['website-argocd-k8s-githubactions-kustomize-kyverno03-service.davtro.svc.cluster.local:80']
