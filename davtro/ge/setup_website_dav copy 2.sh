...
check_microk8s() {
    ...
    if [[ $(microk8s status --field=daemon-running) != "yes" ]]; then
        echo "⚠️ MicroK8s nie jest uruchomiony. Uruchamiam..."
        microk8s start
...
enable_addon() {
    ...
    if ! microk8s status --field=addons | grep -q "${addon_name}"; then
...
- name: postgres-secret
  literals:
  - password=bardzotajnehaslo123 # Hasło do PostgreSQL (zmień!)
...
# ArgoCD Application (Wskazuje na LOKALNY katalog! To jest niestandardowe, 
# ale pozwala to monitorować lokalny katalog w trybie deweloperskim)
...
spec:
  project: default
  # UWAGA: Source wskazuje na lokalny system plików, co jest możliwe 
  # tylko jeśli ArgoCD jest skonfigurowany do czytania z lokalnego repo.
  # Dla standardowego ArgoCD, potrzebny byłby GIT.
  # Użyjemy GIT i po prostu pominiemy push na tym etapie, ale ArgoCD wymaga GIT.
  # Aby wdrożenie się powiodło z MicroK8s bez push, wykonamy bezpośrednio Kustomize
  # i tylko zdefiniujemy Argo App, abyś mógł ją zobaczyć.
  source:
    repoURL: https://github.com/${REPO_OWNER}/${REPO_NAME}.git # Wymagane przez ArgoCD
    targetRevision: HEAD
...
echo "✅ Definicja aplikacji ArgoCD utworzona. (Wymaga pusha do synchronizacji!)"

# --- 9. Wdrożenie bezpośrednie z Kustomize (Opcja lokalna) ---
echo "💾 Bezpośrednie wdrożenie przy użyciu Kustomize (aby pominąć GitSync):"
